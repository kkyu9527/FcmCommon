...
FcmCommon 是一个面向 HyperOS / MIUI 场景的 LibXposed 模块配置应用，用来集中管理 FCM 相关修复、目标应用白名单和模块状态诊断。
...
- 维护白名单，让需要重点保活的应用进入修复链路。
- 查看模块连接状态、作用域状态、最近连接事件和诊断信息。
- 支持二级 / 三级页面和预测返回手势过渡。

## 页面说明

- `总览`：查看模块是否已连接、已启用功能数量、白名单数量和最近一次扫描结果。
- `应用`：搜索应用、查看 FCM 候选、管理白名单、手动重新扫描。
