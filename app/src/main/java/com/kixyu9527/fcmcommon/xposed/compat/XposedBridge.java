...
public final class XposedBridge {
    ...
    public static void log(String text) {
        android.util.Log.i("FcmCommon", text);
    }
    ...
    public static XC_MethodHook.Unhook hookMethod(Member member, XC_MethodHook callback) {
        ...
        HOOKS.computeIfAbsent(member, key -> new ArrayList<>()).add(callback);
    ...
    public static Object invokeOriginalMethod(Member method, Object thisObject, Object[] args) throws Throwable {
        ensureInit();
        if (method instanceof Method targetMethod) {
            XposedInterface.Invoker<?, Method> invoker = xposedInterface.getInvoker(targetMethod);
        ...
        if (method instanceof Constructor<?> constructor) {
            XposedInterface.CtorInvoker<?> invoker = xposedInterface.getInvoker((Constructor<?>) constructor);
            try {
